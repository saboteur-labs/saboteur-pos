...
export function runContextNew(slug: string, options: NewContextOptions): void {
  if (slug === 'inbox') {
    process.stderr.write(c.red("'inbox' is a reserved context name.\n"));
    process.exit(1);
  }
  if (!SLUG_RE.test(slug)) {
    process.stderr.write(c.red('Slug must be lowercase alphanumeric with optional hyphens.\n'));
    process.exit(1);
  }
  ...
  if (getContext(db, slug)) {
    process.stderr.write(c.red(`Context '${slug}' already exists.\n`));
    db.close();
    process.exit(1);
  }
  ...
  db.close();
  process.stdout.write(c.green(`Created context: ${slug}\n`));
}
