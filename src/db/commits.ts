...
export function upsertCommit(db: Database.Database, commit: CommitInput): void {
  db.prepare(
    `INSERT INTO commits (sha, repo, branch, task_id, message, author_ts)
     VALUES (@sha, @repo, @branch, @task_id, @message, @author_ts)
...
export function listCommitsForTask(db: Database.Database, task_id: string): CommitRow[] {
  return db
    .prepare(
      `SELECT sha, repo, branch, task_id, message, author_ts
       FROM commits
       WHERE task_id = ?
